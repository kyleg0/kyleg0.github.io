...
function formatFloat(float, decimals) {
  if (!_.isNull(float) && float.toFixed) 
    return float.toFixed(decimals);
  else 
    return float + "";
}

function Poisson(clients,workload)
{
  //doing this all at once is problematic for large numbers
  var i;
  var result=10000.0
  //We really ought to integrate in the future and solve
  divisionLeft=[]
  divisor=Math.E
  workammount=workload
  //reduce calculation time a bit, 50 is arbitrary
  while(workammount>50)
  {
    workammount=workammount/2
    divisor=divisor*divisor
  }
  
  
  for(i=0;i<clients;i+=10)
  {
    divisionLeft.push(getFactorialPart(i))
  }
  divisionLeft.push(getFactorialPart(clients))
  
  for (i=0;i<workammount;i++)
  {
    //this does the work of e^-workload
    divisionLeft.push(divisor)
  }
  remainder=workammount-i
  divisionLeft.push(Math.pow(divisor,remainder))
  for (i=0;i<clients;i++)
  {
    result=result*workload
    while (result>1 && divisionLeft.length>0)
    {
      result=result/divisionLeft.pop()
    }
  }
  while (divisionLeft.length>0)
  {
    result=result/divisionLeft.pop()
  }
  return result;
  //return Math.pow(Math.E, -workload)*Math.pow(workload,clients)/factorial(clients)
}

function extrasf(currentNum) {
  if (currentNum<4) {
    return 1
...
function getTo95(workload,othernumber) {
  if (othernumber == null) othernumber = 94.5;
  target = othernumber * 100;
  var x = 0.0;
  var clients=0;
  while (x < target) {
    c = Poisson(clients, workload);
    clients += 1;
    x += c;
  }
  return clients - 1
...
//representing 10! 20!/10! 30!/20! and so on
factorialParts=[]

function generatePart(arrayPos)
{
    if(arrayPos==0)
    {
        result=1
    ...
    {
        result=1
        for(var i=arrayPos*10-9;i<arrayPos*10+1;i++)
        {
            result=result*i
        }
    }
    factorialParts.push(result)
    
}

function getFactorialPart(number)
{
    mod=number%10
    if (mod==0)
    {
        arraypos=number/10
        if(arraypos+1>factorialParts.length)
        {
            generatePart(arraypos)
        }
        return factorialParts[arraypos]
    }
    else
    {
        //21 through 26, not 20 through 26
        base=number-mod+1
        result=1
        for(var i=base;i<number+1;i++)
        {
            result=result*i
        }
        return result
...
/*
 * Localization support.  Provides singleton translations oracle.
 */
var Locale = function() {
  ...
  function addLocaleValue(value) {
    var key = value['key'];
    if (strings[key] == null) {
      strings[key] = {};
    }
    strings[key] = _.omit(value, 'key');
  }
  ...
  return {
    ...
    addLocaleValues: function(values) {
      for (var i = 0; i < values.length; i++) {
        addLocaleValue(values[i]);
      }
    },
    ...
    t: function(key) {
      ...
      if (values && values[currentLocale]) {
        ...
        return "Missing " + currentLocale + " translation: '" + key + "'";
      }
    },

    m: function(key, variables) {
      var values = strings[key];
      if (values && values[currentLocale]) {
        return _.template(values[currentLocale])(variables);
      } else {
        return "Missing " + currentLocale + " translation template: '" + key + "'";
      }
    }
